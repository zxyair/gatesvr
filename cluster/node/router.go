...
func (r *Router) AddRouteHandler(route int32, stateful bool, handler RouteHandler, middlewares ...MiddlewareHandler) {
	if r.node.getState() != cluster.Shut {
		log.Warnf("the nodestart server is working, can't add route handler")
		return
	}
...
func (r *Router) SetDefaultRouteHandler(handler RouteHandler) {
	if r.node.getState() != cluster.Shut {
		log.Warnf("the nodestart server is working, can't set default route handler")
		return
	}
...
func (r *Router) SetPreRouteHandler(handler RouteHandler) {
	if r.node.getState() != cluster.Shut {
		log.Warnf("the nodestart server is working, can't set pre-route handler")
		return
	}
...
func (r *Router) SetPostRouteHandler(handler RouteHandler) {
	if r.node.getState() != cluster.Shut {
		log.Warnf("the nodestart server is working, can't set post-route handler")
		return
	}
